...
6. **Goblin Chest**: a chest and a meat pack, shapeless. Mossy green under dark iron bands, with bone horns on the lid and a green eye on the lock plate; when it opens the lid turns out to be a jaw full of teeth. Two of them next to each other make a double chest with a bigger set of horns.
7. **Milk Churn**: iron, glass, iron / iron, bucket, iron / three iron. A grey metal tank that holds ten buckets of milk; the milk shows behind its glass window, and a broken churn keeps its milk (the item shows how much). Right-click with a milk bucket to pour it in, with an empty bucket to fill it; an empty hand opens its screen (the one grey screen of the mod): a bucket slot that pours milk buckets in and fills empty buckets, and a gauge with the level. Hoppers put buckets in from the top and the sides and take them out below; a comparator reads the level.
...
- **Right-click the bed**: Rest, Chop, Farm or Collect, the radius and (for Chop) replanting on or off.
  ...
  - *Farm*: a farmer does whatever its tool row has the tools for, within the radius and never in a home, the nearest thing first. Tools never wear out.
    ...
    - **Bucket**: milk grown cows, each cow once every five minutes. At home the milk goes into a **Milk Churn** (the bucket is used up), what does not fit goes into the goblin chest.
