 ...
 * home (resting, unloading) but stands still outside its flat in a small closed area, it picks a column at the side
 * of the hole, stacks goblin scaffold there and climbs until it can step onto the ground above. A column somebody
 * else already built is used first (it costs no new blocks). The scaffold stays behind and vanishes by itself.
 * When there is no way up at all the goblin blinks home.
 */
public class ClimbOutGoal extends Goal {
    ...
    private enum Stage { WALK, CLIMB, STEP_OUT }
    ...
    @Nullable private Vec3 lastCheckPos;
    ...
    public boolean canUse() {
        ...
        if (bed == null || !bed.owns(goblin.getUUID()) || !wantsHome(bed)) return false;
        ...
        if (flat.isInside(goblin.blockPosition())) return false;
        Vec3 pos = goblin.position();
        boolean moved = lastCheckPos != null && pos.distanceToSqr(lastCheckPos) > 1.0;
        lastCheckPos = pos;
        if (moved) return false; // on its way somewhere, not stuck

        Set<BlockPos> area = closedArea(level, goblin.blockPosition(), flat);
        if (area == null) return false;
        plan = findPlan(level, area);
        if (plan == null) {
    ...
    /**
     * Everything the goblin can walk to from {@code start} (steps of one up, drops of up to three). Returns null when
     * that area is not a closed hole: it is large, reaches far, or touches the goblin's flat.
     */
    @Nullable
    ...
    private Plan findPlan(ServerLevel level, Set<BlockPos> area) {
        ...
        for (BlockPos spot : area.stream().sorted((a, b) -> Double.compare(a.distSqr(start), b.distSqr(start))).toList()) {
            ...
            for (int h = 1; h <= Climber.MAX_HEIGHT; h++) {
                ...
                int newBlocks = Math.max(0, h - existing);
